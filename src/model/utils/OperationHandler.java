...
public class OperationHandler {
		...
		try {
			if (op.equals("Reciproc")) {
				if(!specialOperationInProgress)
					temporaryResult = processOperation("1/"+lb1.getText());
				else
					temporaryResult = processOperation("1/"+temporaryResult);
				handleSpecialOperation(lb1, lb2, "reciproc");
			} else if (op.equals("√")) {
				if(!specialOperationInProgress) 
					temporaryResult = processOperation(String.format("Math.sqrt(%s)",lb1.getText()));
				else 
					temporaryResult = processOperation(String.format("Math.sqrt(%s)",temporaryResult));
				handleSpecialOperation(lb1, lb2, "sqrt");
			} else if (op.equals("x²")) {
				if(!specialOperationInProgress)
					temporaryResult = processOperation(String.format("Math.pow(%s,2)",lb1.getText()));
				else
					temporaryResult = processOperation(String.format("Math.pow(%s,2)",temporaryResult));
				handleSpecialOperation(lb1, lb2, "square");
			} else if (op.equals("x³")) {
				if(!specialOperationInProgress) 
					temporaryResult = processOperation(String.format("Math.pow(%s,3)",lb1.getText()));
				else 
					temporaryResult = processOperation(String.format("Math.pow(%s,3)",temporaryResult));
	...
	private static void handleSpecialOperation(Label lb1, Label lb2, String operation) {
		if(temporaryResult.matches("\\d*[\\.][0]"))
		...
		if(!specialOperationInProgress) {
			...
			lb2.setText(handleMultipleSpecialOperations(lb2,operation));
